...
def crdt_write(master,amt,accnt,name):
        ...
        frec.close()
        messagebox.showinfo("Operation Sucessfull!!","Amount Credited Sucessfully!!")
        master.destroy()
        return
...
def debit_write(master,amt,accnt,name):
        ...
        if(int(amt)>camt):
                ...
                frec.write(str(strftime("[%Y-%m-%d] [%H:%M:%S] ",gmtime()))+"       "+"         "+str(amti)+"             "+str(cb)+"\n")
                frec.close()
                messagebox.showinfo("Operation Sucessfull!!","Amount Debited Sucessfully!!")
                master.destroy()
                return
def Cr_Amt(accnt,name):
        ...
        creditwn.configure(bg="Black")
        frl=tk.Frame(creditwn,bg="White")
        l_title=tk.Message(creditwn,text="UNITED BANK",relief="raised",width=2000,padx=600,pady=0,fg="Black",bg="White",justify="center",anchor="center")
...
def De_Amt(accnt,name):
        ...
        debitwn.configure(bg="Black")
        frl=tk.Frame(debitwn,bg="white")
...
def disp_tr_hist(accnt):
        ...
        l_title=tk.Message(disp_wn,text="UNITED BANK",relief="raised",width=2000,padx=600,pady=0,fg="black",bg="white",justify="center",anchor="center")
        l_title.config(font=("courier","50","bold"))
        l_title.pack(side="top")
...
def logged_in_menu(accnt,name):
        ...
        fr1=tk.Frame(rootwn)
        fr1.pack(side="top")
        l_title=tk.Message(rootwn,text="SIMPLE BANKING\n SYSTEM",relief="raised",width=2000,padx=600,pady=0,fg="black",bg="white",justify="center",anchor="center")
        l_title.config(font=("courier","50","bold"))
        l_title.pack(side="top")
        label=tk.Label(text="Logged in as: "+name,relief="raised",bg="white",fg="black",anchor="center",justify="center")
        label.pack(side="top")
        ...
        b2=tk.Button(image=myimg2,relief="raised",command=lambda: Cr_Amt(accnt,name))
        b2.image=myimg2
        b3=tk.Button(image=myimg3,relief="raised",command=lambda: De_Amt(accnt,name))
        b3.image=myimg3
        b4=tk.Button(image=myimg4,relief="raised",command=lambda: disp_bal(accnt))
        b4.image=myimg4
        b5=tk.Button(image=myimg5,relief="raised",command=lambda: disp_tr_hist(accnt))
        b5.image=myimg5
...
def log_in(master):
        ...
        loginwn.config(bg="black")
        fr1=tk.Frame(loginwn,bg="white")
        l_title=tk.Message(loginwn,text="UNITED BANK",relief="raised",width=2000,padx=600,pady=0,fg="black",bg="white",justify="center",anchor="center")
        l_title.config(font=("Garamond","50","bold"))
        l_title.pack(side="top")
        ...
        e3.pack(side="top")
        b1=tk.Button(loginwn,text="SUBMIT",command=lambda: check_log_in(loginwn,e1.get().strip(),e2.get().strip(),e3.get().strip()))
        b1.pack(side="top")
        b2=tk.Button(text="HOME",relief="raised",bg="white",fg="black",command=lambda: home_return(loginwn))
        b2.pack(side="top")
...
def Create():
        ...
        l_title=tk.Message(crwn,text="UNITED BANK",relief="raised",width=2000,padx=600,pady=0,fg="black",bg="white",justify="center",anchor="center")
        l_title.config(font=("Garamond","50","bold"))
        l_title.pack(side="top")
